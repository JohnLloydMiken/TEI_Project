...
// ── Action Button — only opens modal, never calls onClaim directly ─────────
const ActionButton = ({
  status,
  isClaiming,
  onClick,
          ...
          <div className="flex items-center gap-3">
            <div
              className={`p-2 rounded-xl ${
                isUntagging ? "bg-amber-50 text-amber-600" : "bg-emerald-50 text-emerald-600"
              }`}
            >
...
export default function CustomerTable({
  ...
  return (
    <>
      {/* Shared confirm modal — one instance for the whole table */}
      <AnimatePresence>
        {pendingClaim && (
          <ConfirmModal
            pending={pendingClaim}
              ...
              {customers.map((c) => (
                <tr key={c.id} className="border-b border-gray-100 hover:bg-[#f8fafc] transition-colors">
                  <td className="px-4 py-4">
                    <span className="font-mono text-sm font-medium text-teiblue">{c.accountNo}</span>
                  </td>
                  <td className="px-4 py-4">
                    <span className="font-mono text-sm font-medium text-teiblue">{c.accountCode}</span>
                  </td>
                  <td className="px-4 py-4">
                    <span className="text-sm font-semibold text-gray-800">{c.customerName}</span>
                  </td>
                  <td className="px-4 py-4">
